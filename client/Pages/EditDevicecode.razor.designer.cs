...
namespace Blackmail.Pages
{
    public partial class EditDevicecodeComponent : ComponentBase
    {
        ...
        {
            var azureBlackmailGetDevicecodeByUserCodeResult = await AzureBlackmail.GetDevicecodeByUserCode(userCode:$"{UserCode}");
            devicecode = azureBlackmailGetDevicecodeByUserCodeResult;
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Blackmail.Models.AzureBlackmail.Devicecode args)
        {
            try
            {
                var azureBlackmailUpdateDevicecodeResult = await AzureBlackmail.UpdateDevicecode(userCode:$"{UserCode}", devicecode:devicecode);
                DialogService.Close(devicecode);
            }
            catch (System.Exception azureBlackmailUpdateDevicecodeException)
            {
                NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to update Devicecode");
            }
        }
