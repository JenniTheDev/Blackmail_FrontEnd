...
namespace Blackmail.Pages
{
    public partial class AddPersistedgrantComponent : ComponentBase
    {
        ...
        {
            persistedgrant = new Blackmail.Models.AzureBlackmail.Persistedgrant(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Blackmail.Models.AzureBlackmail.Persistedgrant args)
        {
            try
            {
                var azureBlackmailCreatePersistedgrantResult = await AzureBlackmail.CreatePersistedgrant(persistedgrant:persistedgrant);
                DialogService.Close(persistedgrant);
            }
            catch (System.Exception azureBlackmailCreatePersistedgrantException)
            {
                NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to create new Persistedgrant!");
            }
        }
